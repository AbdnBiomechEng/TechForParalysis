function runopt_DS
% Set up and run a movement optimisation using Direct Shooting
...
data = readtable('input_data/input_data_elbow_flexion.csv');

% For mot files generated by Matlab (make_osimm.m) use importfile_mot
%data = importfile_mot('DSEM_Weighted_model_9.mot');

% For mot files generated by Opensim use import_mot_osim
%data = importfile_mot_osim('DSEM_Weighted_model_9.mot',dofnames);
...
lockeddofs = {'TH_x','TH_y','TH_z','SC_y','SC_z','SC_x','AC_y','AC_z','AC_x','GH_y','GH_z','GH_yy','PS_y'};

% Choose which model parameter file to use
OptSetup.model = 'model_struct.mat';

% Should angular velocities be zero at the end of movement?
% (true/false)
...
OptSetup.N = nodes;
OptSetup.MaxIter = 1;	  % max number of iterations for the optimization
...
OptSetup.Wscap = 0;    % weight for scapulo-thoracic gliding plane 
OptSetup.Whum = 0;     % weight for glenohumeral stability 
...
% Filename
filename = [folder_name '/elbow_flexion'];
...
%% Run optimisation
das3_optimize_DS(data,[filename '_' num2str(nodes)],OptSetup);
