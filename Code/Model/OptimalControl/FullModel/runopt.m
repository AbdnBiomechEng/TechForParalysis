...
data = readtable('input_data_high_reach.csv');

% For mot files generated by Matlab (make_osimm.m) use importfile_mot
data = importfile_mot('high_reach/healthy_5.mot');
data_thor = load('high_reach/healthy_5');
% data.Trunk_Hum_y = data_thor.Result.thor_hum(1,:)';
% data.Trunk_Hum_z = data_thor.Result.thor_hum(2,:)';
% data.Trunk_Hum_yy = data_thor.Result.thor_hum(3,:)';
data.Hand_position_x = data_thor.Result.hand_pos(1,:)';
data.Hand_position_y = data_thor.Result.hand_pos(2,:)';
data.Hand_position_z = data_thor.Result.hand_pos(3,:)';
...
input_variables = {'time','TH_x','TH_y','TH_z','SC_z','Hand_position_x','Hand_position_y','Hand_position_z'};
%input_variables = {};
...
OptSetup.model = 'simplified_model_struct_C5.mat';
...
% Set number of nodes
maxnodes = 5;		% end close to this number of nodes
...
% Cost function
OptSetup.Wdata = 0;%10;        % weight for the kinematic term in the cost function
OptSetup.Weffort = 0;%1;       % weight for the energy consumption term in the cost function
OptSetup.Wscap = 0;%0.1;       % weight for scapulo-thoracic gliding plane (if missing, assumed to be constraint)
OptSetup.Whum = 0;          % weight for glenohumeral stability (if missing, assumed to be constraint)

% Add muscle weakness due to injury
OptSetup.max_act_table = readtable('max_act_C5.csv');

% Folder for results (it will be created if it doesn't already exist)
folder_name = 'high_reach';
% Filename
filename = [folder_name '/C5_hand_notrack'];
