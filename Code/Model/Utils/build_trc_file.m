function build_trc_file(filename,markers)
% Build_trc_file creates a .trc file that can be used as input to the
% OpenSim model das3.osim
%
% Input
%     filename = the complete path and name of the .csv file that we want
...
%                           das3.osim are used (see line 16 below)

[pathstr,name,~] = fileparts(filename);


if nargin<2
    markers = {'C7','T8','IJ','PX','AA','AC','AI','TS','SC','GH','EL','EM','US','RS'};
end
...
TRC_filename = fullfile(pathstr,[name '.trc']);
...
for imark = 1:length(markers)
    ...
    if (isfield(data.marker_data.Markers,markers{imark}))
        ...
        % Canadian lab coordinate frame is x forward, y up, z right. Convert to x
        % right, y up, z back for OpenSim model
        %data_out = [data_out; XYZ(:,3)'/1000; XYZ(:,2)'/1000; -XYZ(:,1)'/1000];
