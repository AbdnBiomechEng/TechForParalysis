...
fileDasName = 'model-das3_thor_newmarkerset.osim'
...
output_osim_file = '1.model_scaled_bones_and_muscles.osim'

for muscle in list_musclesNames(tree_osim):
    this_muscle = get_muscle(tree_osim, muscle) # locate muscle
    fmax_original = float(this_muscle.find("max_isometric_force").text) # get value
    this_muscle.find("max_isometric_force").text = str(fmax_original * scale_factor) # change value
...
# Save as a new file
try:
    tree_osim.write(CURR_DIR+'/'+output_osim_file, xml_declaration=True, encoding='utf-8') # save with the header
...
#model_scaled.updVisualizer().updSimbodyVisualizer().setShutdownWhenDestructed(True)
model_scaled.updVisualizer().updSimbodyVisualizer().shutdown() # allows to close with escape key
import time
#while 1:
#    time.sleep(1)
#    model_scaled.getVisualizer().show(state)
